...
from apps.recruitment.serializers.serializers_v1 import *
...
class RecruitmentViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response({'message': 'Recruitment created successfully'}, status=status.HTTP_201_CREATED)
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not instance:
            return Response({'message': 'Invalid Recruitment'}, status=status.HTTP_400_BAD_REQUEST)
        serializer = self.serializer_class(instance=instance, data=request.data)
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response({'message': 'Recruitment updated successfully'}, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
